...
export function Orders() {
  ...
  function handleTakeOrder(id) {
    axios
      .get(`http://127.0.0.1:8000/api/customer/received/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  function handleCancel(id) {
    axios
      .get(`http://127.0.0.1:8000/api/customer/cancel_order/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
          ...
          <div className="d-flex flex-column gap-2 align-items-center w-100">
            {orders.map((e) => {
              return (
                <div
                  key={e.id}
                  className="cart d-flex gap-3 justify-content-between w-100 container bg-success bg-opacity-25 rounded-4 p-3"
                >
                  <div className="w-100 d-flex flex-column align-items-start">
                    ...
                    {e.status === "pending" && (
                      <>
                        ...
                        <div className="d-flex align-items-center p-2 gap-2">
                          <CheckCircleOutline />
                          <p className="m-0">
                            Waiting for delivery to be assigned
                          </p>
                        </div>
                        <div
                          style={{
                        ...
                        <div className="d-flex align-items-center p-2 gap-2">
                          <CheckCircleOutline />
                          <p className="m-0">You received the order</p>
                        </div>
                      </>
                    )}
                    ...
                    {e.status === "assigned" && (
                      <>
                        ...
                        >
                          ...
                          <p className="m-0">Waiting for delivery to be assigned</p>
                        </div>
                        <div
                          style={{
                            width: "1px",
                    ...
                    {e.status === "canceled" && (
                      <div className="d-flex justify-content-center align-items-center h-75">
                        <p className="alert alert-danger"> {e.status}</p>
                      </div>
                    )}
                    ...
                    {e.status !== "canceled" && (
                      <button
                        ...
                        disabled={e.status === "delivered" || e.status === "received"}
                        className="btn-danger btn my-3"
                      >
                        Cancel
                      </button>
                    )}
                  ...
                  <div className="w-100">
                    ...
                    >
                      <div>
                        <p className="fw-bolder text-center">Total : {e.total_price}</p>
                        <p className="fw-bold text-center">Phone : {e.phone}</p>
                        <p className="fw-bold text-center">Address : {e.address}</p>
                        <p className="fw-bold text-center">Delivery ID : {e.delivery_id || "Wait"}</p>
                        <p className="text-center">Time : {new Date(e.created_at).toISOString().slice(2, 10)}</p>
                      </div>
                    </div>
